...
from ootd.utils_ootd import get_mask_location
...
example_path = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'examples')
...
def on_ui_tabs():
    ...
    with block as ootdiddusion_interface:
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                category_dc = gr.Dropdown(label="Garment category (important option!!!)",
                                          choices=["Upper-body", "Lower-body", "Dress"], value="Upper-body")
                example = gr.Examples(
            ...
            with gr.Column():
                result_gallery_dc = gr.Gallery(label='Output', show_label=False, elem_id="gallery", preview=True,
                                               scale=1)
        with gr.Column():
    ...
    return [(ootdiddusion_interface, "Ootdiffusion", "Ootdiffusion")]
